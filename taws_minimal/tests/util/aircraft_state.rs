...
impl<'a> Arbitrary<'a> for AircraftStateWrapper {
    fn arbitrary(u: &mut Unstructured<'a>) -> arbitrary::Result<Self> {
        ...
        *state.timestamp_mut() =
            Time::new::<time::second>(<i32 as Arbitrary>::arbitrary(u)? as f64);
        *state.position_latitude_mut() =
            Angle::new::<angle::degree>(<i32 as Arbitrary>::arbitrary(u)? as f64);
        *state.position_longitude_mut() =
            Angle::new::<angle::degree>(<i32 as Arbitrary>::arbitrary(u)? as f64);
        *state.altitude_mut() =
            Length::new::<length::foot>(<i32 as Arbitrary>::arbitrary(u)? as f64);
        *state.altitude_ground_mut() =
            Length::new::<length::foot>(<i32 as Arbitrary>::arbitrary(u)? as f64);
        *state.speed_ground_mut() =
            Velocity::new::<velocity::knot>(<i32 as Arbitrary>::arbitrary(u)? as f64);
        *state.speed_air_mut() =
            Velocity::new::<velocity::knot>(<i32 as Arbitrary>::arbitrary(u)? as f64);
        *state.climb_rate_mut() =
            Velocity::new::<velocity::foot_per_minute>(<i32 as Arbitrary>::arbitrary(u)? as f64);
        *state.heading_mut() =
            Angle::new::<angle::degree>(<i32 as Arbitrary>::arbitrary(u)? as f64);
        *state.track_mut() = Angle::new::<angle::degree>(<i32 as Arbitrary>::arbitrary(u)? as f64);
